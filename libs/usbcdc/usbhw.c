...
#define SCB_SYSAHBCLKCTRL_GPIO                    ((unsigned int) 0x00000040)
#define SCB_SYSAHBCLKCTRL_CT32B1                  ((unsigned int) 0x00000400)
#define SCB_SYSAHBCLKCTRL_IOCON                   ((unsigned int) 0x00010000)
#define SCB_SYSAHBCLKCTRL_USB_REG                 ((unsigned int) 0x00004000)
...
void USBIOClkConfig( void )
{
  ...
  LPC_SYSCON->SYSAHBCLKCTRL |= (SCB_SYSAHBCLKCTRL_CT32B1 | SCB_SYSAHBCLKCTRL_IOCON | SCB_SYSAHBCLKCTRL_USB_REG);

  // Setup USB clock
  LPC_SYSCON->PDRUNCFG &= ~(SCB_PDSLEEPCFG_USBPAD_PD);        // Power-up USB PHY
  LPC_SYSCON->PDRUNCFG &= ~(SCB_PDSLEEPCFG_USBPLL_PD);        // Power-up USB PLL

  LPC_SYSCON->USBPLLCLKSEL = SCB_USBPLLCLKSEL_SOURCE_MAINOSC; // Select PLL Input
  LPC_SYSCON->USBPLLCLKUEN = SCB_USBPLLCLKUEN_UPDATE;         // Update Clock Source
  LPC_SYSCON->USBPLLCLKUEN = SCB_USBPLLCLKUEN_DISABLE;        // Toggle Update Register
  LPC_SYSCON->USBPLLCLKUEN = SCB_USBPLLCLKUEN_UPDATE;

  // Wait until the USB clock is updated
  while (!(LPC_SYSCON->USBPLLCLKUEN & SCB_USBPLLCLKUEN_UPDATE));

  // Set USB clock to 48MHz (12MHz x 4)
  LPC_SYSCON->USBPLLCTRL = (SCB_USBPLLCTRL_MULT_4);
  while (!(LPC_SYSCON->USBPLLSTAT & SCB_USBPLLSTAT_LOCK));    // Wait Until PLL Locked
  LPC_SYSCON->USBCLKSEL = SCB_USBCLKSEL_SOURCE_USBPLLOUT;

  // Set USB pin functions
  LPC_IOCON->PIO0_3 &= ~IOCON_PIO0_3_FUNC_MASK;
  LPC_IOCON->PIO0_3 |= IOCON_PIO0_3_FUNC_USB_VBUS;         // VBus
  LPC_IOCON->PIO0_6 &= ~IOCON_PIO0_6_FUNC_MASK;
  LPC_IOCON->PIO0_6 |= IOCON_PIO0_6_FUNC_USB_CONNECT;      // Soft Connect
